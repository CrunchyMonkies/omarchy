...
| `GALLIUM_DRIVER=d3d12` and `LIBVA_DRIVER_NAME=d3d12` when `/dev/dxg` and Mesa's d3d12 driver are both present, `LIBGL_ALWAYS_SOFTWARE=1` and llvmpipe when they are not | `bin/omarchy-launch-wsl-session` | WSL shares the host GPU through `/dev/dxg`, which is a char device and so untouched by the render-node rule. Mesa cannot auto-detect it with no DRM render node to probe, so the driver has to be named. `OMARCHY_WSL_SOFTWARE_RENDER=1` forces software back |
