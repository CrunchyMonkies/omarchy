...
| `LIBGL_ALWAYS_SOFTWARE=1`, `GALLIUM_DRIVER=llvmpipe` for the compositor | `bin/omarchy-launch-wsl-session` | It allocates through GBM on VKMS, and a GBM device uses whichever driver Mesa was named. Point it at the GPU and allocation fails with `bo null`, the swapchain never fills, and the desktop draws zero rects |
| `GALLIUM_DRIVER=d3d12` and `LIBVA_DRIVER_NAME=d3d12` for applications, on the `OMARCHY_WSL_GPU` marker | `install/wsl/uwsm.sh`, `bin/omarchy-launch-wsl-session` | WSL shares the host GPU through `/dev/dxg`, a char device untouched by the render-node rule. Applications render into their own buffers, so nothing of theirs lives on VKMS. Every application passes through `uwsm-app`, so the switch sits there. `OMARCHY_WSL_SOFTWARE_RENDER=1` forces software back |
